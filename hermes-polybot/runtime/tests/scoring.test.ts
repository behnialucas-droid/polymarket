...
function mkTrade(over: Partial<WalletTrade> = {}): WalletTrade {
  return { walletAddress: '0xabc', marketId: 'm1', outcome: 'YES', side: 'BUY', price: 0.5, size: 100, timestamp: new Date().toISOString(), marketCategory: 'politics', ...over };
}
function mkMarket(over: Partial<MarketData> = {}): MarketData {
  return { marketId: 'm1', yesPrice: 0.52, noPrice: 0.48, spread: 0.02, liquidity: 20000, timeToResolutionHours: 100, resolved: false, ...over };
}
...
test('copyability: illiquid wide-spread wallet scores low', () => {
  ...
  const good = scoreWallet(Array.from({ length: 10 }, (_, i) => item(0.2, { marketId: `m${i}` }, { liquidity: 80000, spread: 0.01, timeToResolutionHours: 200 })));
...
test('wallet status: thresholds drive track/watch/ignore', () => {
  const good = scoreWallet(Array.from({ length: 15 }, (_, i) => item(0.3, { marketId: `m${i}` }, { liquidity: 80000, spread: 0.01, timeToResolutionHours: 300 })));
...
test('trade scoring: clean setup on strong wallet = paper_copy with $5-$20 size', () => {
  const d = scoreTrade(mkTrade(), mkMarket(), strongWallet, DEFAULT_RULES);
...
test('trade scoring: illiquid market gated to skip', () => {
  const d = scoreTrade(mkTrade(), mkMarket({ liquidity: 10 }), strongWallet, DEFAULT_RULES);
  assert.equal(d.decision, 'skip');
  assert.ok(d.risks.some((r) => r.includes('liquidity')));
});

test('trade scoring: wide spread gated to skip', () => {
  const d = scoreTrade(mkTrade(), mkMarket({ spread: 0.2 }), strongWallet, DEFAULT_RULES);
  assert.equal(d.decision, 'skip');
  assert.ok(d.risks.some((r) => r.includes('spread')));
});

test('trade scoring: price moved too far = skip (too late)', () => {
  const d = scoreTrade(mkTrade({ price: 0.3 }), mkMarket({ yesPrice: 0.6 }), strongWallet, DEFAULT_RULES);
  assert.equal(d.decision, 'skip');
  assert.ok(d.risks.some((r) => r.includes('price moved')));
});

test('trade scoring: weak wallet gated to skip', () => {
  const d = scoreTrade(mkTrade(), mkMarket(), { ...strongWallet, globalScore: 0.2 }, DEFAULT_RULES);
  assert.equal(d.decision, 'skip');
});

test('trade scoring: resolved market gated to skip', () => {
  const d = scoreTrade(mkTrade(), mkMarket({ resolved: true }), strongWallet, DEFAULT_RULES);
  assert.equal(d.decision, 'skip');
});
